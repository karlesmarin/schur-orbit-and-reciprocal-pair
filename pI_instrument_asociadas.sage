...
#
# DE DONDE SALE, leyendo el articulo ANTES de proponer nada (que es la leccion de esta noche).
#   * (2186)  Psi_r = 0  <=>  m_nu(lambda) = m_{nu*}(lambda)  para todo nu, con
#             m_nu(lambda) = sum_{delta PAR} c^lambda_{nu,delta}   (Littlewood GL -> O, filas pares)
#             y nu* la asociada de Littlewood: la primera COLUMNA pasa de nu'_1 a N - nu'_1.
#   * (conj:iso)  el certificado del «testigo aislante» FALLA: en lambda=(5,5,2,1) toda mu tiene
#             EXACTAMENTE DOS contribuyentes, mu y mu*.  O sea que el caso generico es justo
#             C_mu = c_mu -+ c_{mu*}, y todo se reduce a esa igualdad.
#   * (prob:instrument)  «whether that recursion computes them is WHAT WE HAVE NOT ATTEMPTED», y
#             «is C_mu(lambda)=0 an identity between generalized LR coefficients that the flag
#             conditions make visible?  That is a question about tableaux, which is the form in
#             which we would like to see it answered.»
#
