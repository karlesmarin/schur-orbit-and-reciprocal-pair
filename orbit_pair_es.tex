...
% Version en castellano de "Schur polynomials twisted by roots of unity and reciprocal pairs:
% exactly three factors, and where they vanish". Traduccion completa, no resumen: arXiv exige el articulo entero en ambos
...
\hypersetup{
  pdftitle={Polinomios de Schur torcidos por raices de la unidad y pares reciprocos: exactamente tres factores, y donde se anulan},
...
% Sin \\, por la misma razón que la edición inglesa: cualquier corte forzado deja la línea corta
% EN MEDIO, y partiendo en los dos puntos abría renglón con un signo de puntuación. Dejando que
% LaTeX parta, la línea corta cae al final. Comprobado sobre el PDF.
{Polinomios de Schur torcidos por raíces de la unidad y pares recíprocos: exactamente tres factores, y dónde se anulan}
...
las $t$ filas congeladas con un lema de cancelación, y da el signo, del que el de ordenación de Littlewood es un factor.
...
En la literatura de factorización que aquí repasamos las dos líneas no parecen haber compartido
alfabeto, y el obstáculo es estructural en ambas direcciones. El
...
comprobaciones numéricas, la Sección \ref{sec:attr} separa lo que usamos de lo que reclamamos,
enunciado a enunciado, la Sección \ref{sec:open} plantea los problemas abiertos, y la \S\ref{sec:closing} cierra donde
empezó la introducción.

Dos de esos problemas se han movido desde la primera versión, y los dos movimientos se registran
donde los problemas están y no se anuncian aquí como resultados. El Problema \ref{prob:versch}
preguntaba si los dos formalismos --- la Verschiebung $\varphi_t$ en la mitad torcida, los caracteres
universales en la recíproca --- componen. Componen, y la Observación \ref{rem:composes} escribe la
composición; es el Teorema 3.1 de Albion evaluado en $1$ y no un teorema nuestro, y corrige además
una frase de ese problema que declaraba más ignorancia de la debida, porque el operador con una letra
libre del que decía que no conocíamos nada está en \cite[\S6.1]{Alb23}, la referencia que el propio
problema nombra. Tres de ellos los toca además el artículo compañero \cite{PaperII}, que continúa éste
más allá de $r=1$; lo que mueve se dice donde cada problema está, con los límites que ese
artículo le pone, y no se repite aquí. Y el análogo simpléctico \emph{clásico} del Problema \ref{prob:types} es el
\cite[Teorema 2.8]{Kum24}, cuyo criterio de anulación hemos contrastado con el nuestro.
...
el signo total $\varepsilon_\lambda$ & el signo global\\
\bottomrule
\end{tabular}
\end{center}
...
El signo \eqref{eq:sign} refina el signo de ordenación del Teorema
...
\subsection{Más allá de \texorpdfstring{$t=2$}{t=2}: dónde puede vivir una involución así}\label{sec:lift}

El ensamblaje anterior es un enunciado sobre $t=2$, y la razón es la Proposición
\ref{rem:involution}: es un enunciado sobre una palabra, y esa palabra es la de $t=2$. Lo que sigue
no es esa construcción para $t$ general. Es la pregunta más pequeña de \emph{dónde} podría vivir una
involución de la especie requerida --- que sí tiene respuesta, y negativa para el candidato obvio.

Fíjense $t$ y $\lambda$. Combinando la identidad de ramificación \eqref{eq:composes} de la
Observación \ref{rem:composes} con el desarrollo en dos letras de $s_{\lambda/\mu}(z,\zb)$ se obtiene
\begin{equation}\label{eq:Jlambda}
\Phi_t(\lambda;z)\;=\;\sum_{(\mu,\nu)\in J_\lambda}\sgn_t(\mu)\;z^{\,2|\nu|-|\mu|-|\lambda|},
\end{equation}
donde $J_\lambda$ es el conjunto de pares $\mu\subseteq\nu\subseteq\lambda$ tales que
$\core_t(\mu)=\varnothing$, cada componente de $\quot_t(\mu)$ tiene a lo sumo una fila, y tanto
$\nu/\mu$ como $\lambda/\nu$ son bandas horizontales. Ése es el conjunto de términos que presenta la
enumeración, y la involución que pide es una sobre $J_\lambda$: que conserve el peso, invierta el
signo fuera de sus puntos fijos, y tenga por puntos fijos las transversales del Lema \ref{lem:L3}.
Las dos primeras condiciones están disponibles; la tercera no, y el obstáculo es un recuento y no
una falta de ingenio.

\begin{lemma}[la cota estándar, espacio de peso a espacio de peso]\label{lem:fixbound}
Sea $X$ un conjunto signado finito graduado por peso, y escríbase $n_\pm(e)$ para el número de sus
elementos de peso $e$ y signo $\pm$. Si $\iota$ es una involución sobre $X$ que conserva el peso e
invierte el signo fuera de sus puntos fijos, entonces
\[
\bigl|\Fix(\iota)\bigr|\;\ge\;\sum_e\bigl|n_+(e)-n_-(e)\bigr|,
\]
con igualdad si y sólo si, en cada espacio de peso, todos los puntos fijos llevan el mismo signo.
\end{lemma}

\noindent Nada de esto es nuevo. Que una involución que invierte el signo reduce una suma signada a
sus puntos fijos es el recurso estándar, y la cota es esa identidad aplicada espacio de peso a
espacio de peso junto con la desigualdad triangular; la enunciamos porque el resto de la subsección
va del número que produce.

\begin{proof}
Fíjese $e$ y sea $F_\pm$ el número de puntos fijos de peso $e$ y signo $\pm$. Cada $2$-ciclo de
$\iota$ vive dentro de un espacio de peso y retira un elemento de cada signo, así que
$n_+(e)-n_-(e)=F_+-F_-$, de donde $F_++F_-\ge|F_+-F_-|=|n_+(e)-n_-(e)|$, con igualdad exactamente
cuando $F_+$ o $F_-$ se anula. Sumando en $e$ salen los dos enunciados.
\end{proof}

Aplicado a $J_\lambda$ a través de \eqref{eq:Jlambda}, el acotamiento dice
$|\Fix(\iota)|\ge\|\Phi_t(\lambda;z)\|_1$, la suma de los valores absolutos de los coeficientes. Así
que $\|\Phi_t(\lambda;z)\|_1$ es el número \emph{mínimo} de puntos fijos de cualquier involución de
esa especie sobre $J_\lambda$, y el mínimo se alcanza: emparejando $+$ con $-$ de cualquier manera
dentro de cada espacio de peso quedan exactamente esos. No es una cantidad en la que una construcción
pueda ahorrar ---
el primer cruce, el último o cualquier otro---. No es una cantidad en la que una construcción pueda
ahorrar.

\begin{proposition}[$J_\lambda$ es el conjunto equivocado]\label{prop:wrongset}
Por el Lema \ref{lem:L3} el numerador $D_t(z)\Phi_t(\lambda;z)$, con
$D_t(z)=(z^t-1)(z^{-t}-1)(z-z^{-1})$, es una suma signada de $|\mathcal U|\le4$ términos
$f(\beta_{j_1}-\beta_{j_2})$, o sea de a lo sumo ocho monomios. Pero $\|\Phi_t(\lambda;z)\|_1$ no
está acotado por ocho. En $t=2$ y $\lambda=(7,4)$ el conjunto beta es $(10,6,1,0)$, el perfil es de
tamaño tres con triple $(d_1,d_2,d_3)=(4,6,10)$, y por el Corolario \ref{cor:chars}
$\Phi_2=\pm\chi_1\chi_2\chi_4$, un producto de caracteres con todos los coeficientes de un signo,
así que
\[
\|\Phi_2\|_1=\chi_1(1)\chi_2(1)\chi_4(1)=2\cdot3\cdot5=30 .
\]
Esa sola forma lo zanja, y el barrido dice cuánto abunda: sobre $t=2,3$ y $\lambda_1\le7$ la norma
llega a $30$ y a $21$, y pasa de ocho en $128$ de las $329$ formas y en $216$ de las $791$
\stverif. Para esas $\lambda$ ninguna
involución sobre $J_\lambda$ de la especie anterior puede tener por puntos fijos los objetos del
Lema \ref{lem:L3}.
\end{proposition}

Los dos recuentos son de objetos que están a lados opuestos del denominador, y ése es todo el
diagnóstico. $J_\lambda$ indexa el cociente; las transversales del Lema \ref{lem:L3} indexan el
numerador; y una involución sobre $J_\lambda$ tendría que ocultar precisamente el factor $D_t$ que
los separa. El remedio es multiplicar el conjunto de índices por ese factor en vez de dividir los
objetos por él, y mantener el factor \emph{factorizado}, para que cada término conserve su
procedencia.

\begin{definition}\label{def:lift}
Sea $\Omega_t$ el conjunto signado de las ocho elecciones de un término en cada factor de $D_t$,
\[
(z^t-1)\mapsto(t,+1)\ \text{o}\ (0,-1),\qquad
(z^{-t}-1)\mapsto(-t,+1)\ \text{o}\ (0,-1),\qquad
(z-z^{-1})\mapsto(1,+1)\ \text{o}\ (-1,-1),
\]
siendo cada par un peso y un signo, que escribimos $d(\omega)$ y $\sgn(\omega)$. El
\emph{levantamiento al numerador} es $\widehat J_\lambda:=J_\lambda\times\Omega_t$, con
\[
\wt\bigl((\mu,\nu),\omega\bigr)=2|\nu|-|\mu|-|\lambda|+d(\omega),
\qquad
\sgn\bigl((\mu,\nu),\omega\bigr)=\sgn_t(\mu)\,\sgn(\omega).
\]
\end{definition}

\begin{lemma}\label{lem:liftid}
$\sum_{x\in\widehat J_\lambda}\sgn(x)z^{\wt(x)}=D_t(z)\,\Phi_t(\lambda;z)$, y
$|\widehat J_\lambda|=8|J_\lambda|$, con los dos signos igual de numerosos.
\end{lemma}

\begin{proof}
Una suma signada sobre un producto de conjuntos signados es el producto de las sumas; la suma sobre
$\Omega_t$ es $D_t(z)$, factor a factor, y la suma sobre $J_\lambda$ es $\Phi_t(\lambda;z)$ por
\eqref{eq:Jlambda}. Cada factor de $D_t$ aporta un signo de cada clase, así que los ocho elementos de
$\Omega_t$ se reparten cuatro y cuatro.
\end{proof}

\begin{proposition}[sobre el levantamiento el recuento cuadra, y exactamente]\label{prop:liftcount}
Escríbase $\mathcal L(\lambda)$ para el multiconjunto \emph{signado} que sale de desarrollar los
términos del Lema \ref{lem:L3}, llevando cada monomio el producto del signo $\varepsilon_U$ de su
término de Laplace por el signo del término elegido de $f(\beta_{j_1}-\beta_{j_2})$, de modo que
$|\mathcal L(\lambda)|=2|\mathcal U(\lambda)|$. Si $\Phi_t(\lambda;z)\ne0$, entonces
\[
\bigl\|D_t(z)\,\Phi_t(\lambda;z)\bigr\|_1\;=\;2\,|\mathcal U(\lambda)| .
\]
\end{proposition}

\begin{proof}
Como $(z^t-1)(z^{-t}-1)=-f(t/2)^2$ y $D_t=(z^t-1)(z^{-t}-1)(z-z^{-1})=-f(t/2)^2f(1)$, la ecuación
\eqref{eq:main} da
\[
D_t(z)\,\Phi_t(\lambda;z)\;=\;-\varepsilon_\lambda\,f(d_1/2)\,f(d_2/2)\,f(d_3/2).
\]
Al desarrollar, los ocho monomios llevan exponentes $\tfrac12(\pm d_1\pm d_2\pm d_3)$ y por signo el
producto de las tres elecciones. Dos de ellos se cancelan exactamente cuando sus exponentes coinciden
y sus signos difieren, y comparando los dos casos ---que las elecciones difieran en los tres signos o
en uno--- eso sólo ocurre si algún $d_i$ se anula o si uno de $d_1,d_2,d_3$ es la suma de los otros
dos. Los monomios de igual exponente e igual signo se refuerzan, y eso no cambia la norma $\ell^1$.

En el perfil de dos clases $|\mathcal U|=4$ por el Lema \ref{lem:L3}, y no se cancela nada. En
efecto,
\[
2(a_1-b_1)=\tilde d_3+d_1-d_2,\quad
2(a_1-b_2)=\tilde d_3+d_1+d_2,\quad
2(a_2-b_1)=\tilde d_3-d_1-d_2,\quad
2(a_2-b_2)=\tilde d_3-d_1+d_2,
\]
así que cada una de las cuatro sumas $\pm d_1\pm d_2+\tilde d_3$ se anula sólo si algún $a_i$ es
igual a algún $b_j$, imposible porque $A$ y $B$ viven en clases de residuo distintas; las cuatro con
$-\tilde d_3$ son sus opuestas. Y $d_1,d_2\ne0$ porque cada clase lleva dos partes distintas,
mientras que $d_3\ne0$ es la hipótesis $\Phi_t\ne0$ del Teorema \ref{thm:main}(ii). Luego la norma
es $8=2|\mathcal U|$.

En el perfil de tamaño tres $|\mathcal U|=3$, y $A=\{p,q\}$, $B=\{q,r\}$ con $p>q>r$, de modo que
$d_3=p-r=d_1+d_2$. Se cancela exactamente un par, los dos monomios de exponente $0$, y
\[
f(d_1/2)f(d_2/2)f(d_3/2)=z^{d_1+d_2}-z^{d_1}-z^{d_2}+z^{-d_2}+z^{-d_1}-z^{-d_1-d_2},
\]
de norma $6=2|\mathcal U|$, sea $d_1=d_2$ o no. El seis no es un accidente del desarrollo. Escribiendo
$x=z^{d_1/2}$ e $y=z^{d_2/2}$, el producto es $(x-x^{-1})(y-y^{-1})(xy-x^{-1}y^{-1})$, el producto
sobre las tres raíces positivas de $A_2$; la fórmula del denominador de Weyl lo convierte en la suma
alternada sobre $W(A_2)=S_3$, y $6=|W(A_2)|$; esa identidad es la fórmula estándar del
denominador de Weyl para $A_2$.
\end{proof}

La cota $\|D_t\Phi\|_1\le2|\mathcal U|$ sale también del Lema \ref{lem:L3} y la desigualdad
triangular a secas, que es como la vimos primero; lo de arriba es que hay igualdad. Por tanto, por el
Lema \ref{lem:fixbound} aplicado a $\widehat J_\lambda$, una involución allí de la misma especie
deja al menos $2|\mathcal U(\lambda)|$ puntos fijos --- exactamente el número de monomios que
suministra el Lema \ref{lem:L3}, sin holgura por ningún lado. Sobre $t=2,3$ y $\lambda_1\le7$ la
igualdad se midió antes de demostrarse, en $309$ de las $329$ formas y en las $791$, siendo las $20$
excepciones las concéntricas, donde $\Phi_t\equiv0$ \stverif.

Ésa es la mitad de la pregunta que el levantamiento resuelve, y la Figura \ref{fig:lift} es eso. Sobre $J_\lambda$ el requisito es
aritméticamente imposible para un tercio de las formas del rango; sobre $\widehat J_\lambda$ no lo es
para ninguna, y no porque la cota se haya vuelto generosa: para todo $\Phi_t$ no nulo se ha vuelto
ajustada, y son los casos de anulación los que conservan holgura. Lo que el
levantamiento no hace es exhibir la involución, y a eso volvemos en el Problema \ref{prob:toggle}.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig_lift_es.pdf}
\caption{Lo que el levantamiento le hace al recuento. Por el Lema \ref{lem:fixbound} una involución
de la especie requerida deja al menos $\|\Phi_t(\lambda;z)\|_1$ puntos fijos sobre $J_\lambda$ y al
menos $\|D_t\Phi_t(\lambda;z)\|_1$ sobre $\widehat J_\lambda$; las dos se dibujan como
distribuciones sobre las mismas formas, en recuento logarítmico. La línea discontinua está en ocho,
que es el máximo de monomios que el Lema \ref{lem:L3} puede dar, así que lo que hay que leer es la
extensión horizontal y no la forma de ninguno de los dos histogramas: el requisito sin levantar pasa
de la línea ---hasta $30$ en $t=2$ y $21$ en $t=3$, en $128$ y $216$ de las formas--- y el levantado
no puede, tomando sólo los valores $0$, $6$ y $8$. Para $\Phi_t\ne0$ el mínimo levantado es $6$ u
$8$, y la Proposición \ref{prop:liftcount} dice que es exactamente $2|\mathcal U(\lambda)|$; el valor
$0$ corresponde a los casos de anulación, donde esa proposición no se aplica. Sobre el lugar no nulo
el levantamiento no mueve el requisito: lo aplasta exactamente sobre el recuento de Laplace. El azul va hueco sobre el naranja para que los dos se vean donde se solapan, y
la población excluye la partición vacía, como hace la corrida archivada. Calculada por
\texttt{anc/fig\_lift.py}, que reconstruye $J_\lambda$ desde \eqref{eq:Jlambda} y no comparte
código con los guiones de la \S\ref{sec:verif}.}
\label{fig:lift}
\end{figure}

Sí hace, en cambio, que el problema se parta en piezas lo bastante pequeñas para atacarlas una a una.

\begin{corollary}[el requisito es local]\label{cor:local}
Supóngase $\Phi_t(\lambda;z)\ne0$, de modo que se aplica la Proposición \ref{prop:liftcount}. Entonces, para
cada peso $e$,
\[
\bigl|[z^e]\,D_t\Phi_t(\lambda;z)\bigr|
=\#\bigl\{\text{monomios de }\mathcal L(\lambda)\text{ de peso }e\bigr\},
\]
y todos esos monomios llevan el mismo signo. De modo que una involución sobre $\widehat J_\lambda$
puede buscarse espacio de peso a espacio de peso: en el de peso $e$ tiene que dejar fijos exactamente
los monomios de $\mathcal L(\lambda)$ de ese peso, y emparejar todo lo demás.
\end{corollary}

\begin{proof}
Término a término $|[z^e]D_t\Phi|\le\#\{\cdots\}$ por la desigualdad triangular, y por (ii) los dos
miembros tienen la misma suma en $e$ por la Proposición \ref{prop:liftcount}; luego coinciden
para cada $e$. Y la igualdad en una desigualdad
triangular obliga a los sumandos a compartir signo.
\end{proof}

Medido sobre el mismo rango, la forma local vale en $1696$ de $1696$ espacios de peso
desequilibrados en $t=2$ y en $4074$ de $4074$ en $t=3$ \stverif; los $80$ pesos de Laplace que no
llevan desequilibrio en $t=2$ son las $20$ formas con $\Phi_t\equiv0$ contra sus cuatro pesos, y en
$t=3$ no hay ninguno. El contraste es lo que da sentido a todo esto: sobre $J_\lambda$ sin levantar
los coeficientes de $\Phi$ llegan a $7$ y a $3$ en valor absoluto, así que allí el requisito tampoco
es local \stverif.

\begin{remark}[lo que el esqueleto ya obliga]\label{rem:skeleton}
El Corolario \ref{cor:local} deja una construcción por hacer, pero no en todas partes: un espacio de
peso con un solo elemento no admite elección, y allí la aplicación
$\mathcal L(\lambda)\to\widehat J_\lambda$ se puede \emph{leer} en vez de diseñar. Esos espacios son
$398$ de los $1696$ en $t=2$ y $1176$ de los $4074$ en $t=3$, y sobre ellos valen dos cosas sin
excepción \stverif. Primero, el signo del elemento forzado coincide con el del monomio de Laplace de
su peso --- $398$ de $398$ y $1176$ de $1176$ ---, que es condición necesaria para cualquier
involución que cumpla el requisito. Segundo, su $\omega$ toma exactamente uno de los dos factores de
torsión, nunca los dos y nunca ninguno: cuatro de los ocho elementos de $\Omega_t$ quedan excluidos
de entrada, también sin excepción. Tres lecturas naturales fallan sobre los mismos datos, y las
registramos porque cada una habría sido un ansatz plausible: que $\omega$ sea uno de los dos
elementos de peso extremo $\pm(t+1)$ vale en $t=2$ y falla en $70$ de $1176$ casos en $t=3$; que
$\beta(\mu)$ sea $\beta(\lambda)$ sin las dos columnas de la transversal falla en $384$ de $398$ y
en $1092$ de $1176$; y que $\nu$ sea $\mu$ o $\lambda$ falla en $260$ de $398$ y en $860$ de $1176$,
siendo allí $\nu$ estrictamente intermedio.
\end{remark}

La construcción de esta subsección surgió en el curso de una exploración asistida por IA, y sin
ella el artículo no la tendría; la declaración de la \S\ref{sec:disclosure} dice qué significa eso
aquí. La
elección entre las dos lecturas ---que la involución no viva sobre $J_\lambda$ sino sobre un conjunto
de índices del \emph{numerador}--- es suya, y también la razón: una involución sobre $J_\lambda$
tendría que ocultar precisamente el factor que separa el cociente del numerador. Suyo es
$\widehat J_\lambda$ mismo, junto con la instrucción de no desarrollar $D_t$ prematuramente sino
mantenerlo factorizado, una elección signada por factor, para que cada término conserve su
procedencia y los términos de peso coincidente sigan siendo objetos distintos por su etiqueta. Y suya es la corrección que hace verdadero el Lema
\ref{lem:fixbound}: habíamos escrito el número de puntos fijos como una igualdad, y es una
desigualdad, con igualdad exactamente bajo la condición que allí se enuncia. Suya, en fin, es la
observación de que una transversal del Lema \ref{lem:L3} lleva un menor $2\times2$ y por tanto
\emph{dos} monomios, que es lo que convierte un recuento de transversales en la Proposición
\ref{prop:liftcount}. Nuestro es haberlo medido, la localidad del Corolario \ref{cor:local} y el
esqueleto forzado. Se lo agradecemos.

No conocemos un nombre para la construcción. Multiplicar un conjunto signado por otro que represente
los términos de un denominador es el recurso estándar para convertir una identidad entre un cociente
y una suma en una identidad entre dos conjuntos signados, y componer las involuciones resultantes es
el principio de involución de Garsia y Milne \cite{GM81}; lo particular aquí es que $D_t$ se mantiene
factorizado, de modo que la procedencia sobrevive. El formalismo de caminos en el que esas elecciones
son pasos es el de Fulmek \cite{Fulmek}, donde Cauchy--Binet y Laplace ---los dos lados que habría
que emparejar--- se obtienen biyectivamente en el mismo dibujo. Un control dice qué parte de la
construcción hace el trabajo. Recoger las ocho elecciones etiquetadas por procedencia en el polinomio
de Laurent ordinario $D_t$ funde las de igual peso y produce coeficientes de módulo mayor que uno
---en $t=2$, $D_2=-z^3+3z-3z^{-1}+z^{-3}$---, de modo que el objeto recogido no es un conjunto
signado de seis elementos con signos $\pm1$. Recoger los pesos iguales deja intacta la identidad de
polinomios de Laurent del Lema \ref{lem:liftid} y olvida las ocho elecciones etiquetadas por
procedencia que definen $\Omega_t$: el polinomio manda sobre la identidad, y el levantamiento
factorizado sobre la procedencia. Dos
señuelos confirman que el conjunto signado es el correcto: quitar el factor $(z-z^{-1})$ rompe la
identidad en $289$ de $329$ formas y en $623$ de $791$, y dar signo $+1$ a todos los elementos de
$\Omega_t$ la rompe en los mismos números.
...
razones. Es el único de nuestros resultados principales cuya prueba se apoya en
un input externo no elemental, el Teorema \ref{thm:PvW}; y las dos vías que llevan a él no son la
misma vía. La de Littlewood lo reduce, en la
...
letras admiten un solo hueco, luego $s_{(m,n)}(z,\bar z)=S_{m-n+1}$ depende solo de ese hueco. No conocemos
...
autocomplementarias, y no conocemos ninguna para $\mu$ arbitraria.
...
hueco que señala se ha llenado desde entonces por el lado que necesitamos. Para
$GL_n\downarrow O_n$, la primera fórmula combinatoria que extiende del todo la regla de restricción
de Littlewood es la de Jang y Kwon \cite{JK20}, y la atribución la tomamos de Frohmader, que lo dice
con esas palabras; tiene además una propiedad que conviene separar. Escriben
\[
\bigl[V^{\lambda}_{GL_n}:V^{\mu}_{O_n}\bigr]
=\sum_{\delta\in\mathcal P(2)}\overline{c}^{\,\lambda}_{\delta\mu},
\]
su Teorema 1.1, donde $\overline{c}^{\,\lambda}_{\delta\mu}$ \emph{cuenta} las tablas de
Littlewood--Richardson de forma $\lambda/\delta$ y contenido $\mu^{\pi}$ cuya tabla compañera cumple
una condición de bandera, $\tau_j+n_j\le n+1$; su Corolario 4.12 reescribe la cuenta sobre la tabla
ordinaria. La frase «desaparecen en un rango estable» tiene aquí un sentido preciso: cuando
$\ell(\lambda)\le n/2$ las dos sucesiones pasan a ser $m_i=2i-1$ y $n_j=2j$, la desigualdad se cumple
sola, y se recupera la regla de Littlewood --- su Corolario 4.13. Lo que la separa de las extensiones
anteriores es que es una cuenta y no una suma alternada; ellos hacen notar que aquéllas se
obtuvieron por vía algebraica y no son, por tanto, libres de restas, y la suya sí. Frohmader
\cite{Frohmader} da después reglas de ramificación combinatorias para $GL_n\downarrow O_n$ y
$GL_{2n}\downarrow Sp_{2n}$ a la vez, que generalizan las reglas de restricción de Littlewood y son,
en sus palabras, \emph{«válidas para todos los parámetros, es decir, no hay restricciones de rango
estable»}: allí la multiplicidad pasa a ser una suma de coeficientes de Littlewood--Richardson
generalizados $c^{\lambda}_{\mu\nu}(O_n)$ leídos de condiciones de bandera sobre tablas de
Littlewood--Richardson ordinarias, sin ninguna regla de modificación. No usamos ninguna
de las dos abajo, y decimos por qué en un momento; pero son los instrumentos naturales para la única
pregunta que esta sección deja abierta, y volvemos a ellos en el Problema \ref{prob:instrument}.
...
Calculada por \texttt{anc/folding\_t2.py}, cuya corrida archivada imprime cada fila como \emph{cero}
/ \emph{no negativa} / \emph{no positiva} / \emph{MEZCLADA}; la columna \emph{un signo} de aquí es la
suma de las dos clases de signo único, que esa corrida imprime ya también. La figura que la acompaña
es \texttt{anc/fig\_v2.py}, que elige sus dos ejemplos de la misma población pero no produce este
cuadro.}
\label{tab:virtual}
\begin{tabular}{@{}lrrrr@{}}
\toprule
...
\midrule
Teor. \ref{conj:crit}, el criterio, todo $r$ & --- & prueba, \S\ref{sec:rigidity} & \stproved\\
\quad lo mismo, leído sobre $O(N)$, Cor.~\ref{cor:dettwist} & --- &
reformulación, \S\ref{sec:conv} & \stproved\\
\quad Teor. \ref{thm:PvW}, el input externo & --- & \cite{PvW} & \stext\\
\quad Teorema \ref{thm:Ggen}, $|G|\le2$ & $t\le10$, $r\le3$ & voraz contra fuerza bruta, $0/4049$ & \stproved\\
\quad Prop.~\ref{prop:A1gen}, los extremos & $t\le10$, $r\le3$ & $2522$ formas, $0$ fallos & \stproved\\
\quad \quad la hipótesis hace falta & mismo & $414$ formas fallan sin ella & \stproved\\
\quad las identidades mostradas, \S\S\ref{sec:extremal}--\ref{sec:rigidity} & $t=2$, $r\le3$ & $11$ identidades, $0$ fallos & \stverif\\
Lema \ref{lem:step}, movimiento de columna, toda clase & $t\le6$, $r\le3$ &
$285\,600/285\,600$; señuelo de un término $149\,652$ & \stproved\\
Teor. \ref{thm:suff}, la dirección suficiente & $t\le8$, $r\le4$ &
$438$ formas, $0$ fallos; \eqref{eq:wrefl} $438/438$ & \stproved\\
Conj. \ref{conj:general}, ambas direcciones & $t\le12$, $r\le4$ &
$190\,443$ formas, $24$ configuraciones, $0$ fallos & \stverif\\
\quad lo mismo, evaluación independiente & $t\le8$, $r\le4$ &
$43\,010$ formas, $409$ ceros, $0$ fallos & \stverif\\
\quad lo mismo otra vez, en Sage & $t\le8$, $r\le3$ &
$12\,937$ formas, $131$ ceros, $0$ fallos & \stverif\\
\quad contra los Teoremas \ref{thm:main} y \ref{conj:crit} & $r=1$; $t=2$ &
$9913$ y $29\,508$ formas, $0$ discrepancias & \stproved\\
\quad el señuelo que lee $\beta$ en vez de $\mathcal{S}$ & mismo &
falla en $24$ de $24$ y en $14$ de $14$ & \stproved\\
\midrule
Teor. \ref{conj:crit}, ambas direcciones, comprobación directa & $r\le3$, $|\lambda|\le28,26,22$ &
$9961$ formas, $318$ ceros, $0$ fallos & \stproved\\
\quad rama (a), Teor.~\ref{thm:zeros} & $r\le3$, $|\lambda|\le15$ &
$16/16$ & \stproved\\
\quad rama (b), Teor.~\ref{thm:zeros} & $r\le3$ & $24/24$ & \stproved\\
\quad control de anchura par para (b) & $r\le3$ & $56/56$ no nulos & \stproved\\
\quad recíproco en $r=1$, Teor.~\ref{thm:r1} & $\beta_1\le25$ &
$14950$ conjuntos beta, $0$ fallos & \stproved\\
\quad recíproco, $\ell(\lambda)\le N/2$, Teor.~\ref{thm:stable} & $r\le3$ &
$372/372$ testigos & \stproved\\
\quad el testigo aislante, Prop.~\ref{conj:iso} & $r=2$, $|\lambda|\le14$ &
$227$ testigos, $11$ de residuo & \stproved\\
\quad lo mismo en $r=3$ & $r=3$, $|\lambda|\le13$ &
...
\stverif\\
\eqref{eq:Jlambda}, el conjunto de términos de la \S\ref{sec:lift} & $t=2,3$; $\lambda_1\le7$ &
$329/329$ y $791/791$ & \stproved\\
Prop. \ref{prop:wrongset}: $\|\Phi\|_1$ contra el recuento de Laplace & mismo &
$\lambda=(7,4)$ da $30$ exacto; pasa de $8$ en $128$ y $216$ & \stproved\\
Lema \ref{lem:liftid}, el levantamiento & mismo &
$329/329$, $791/791$; dos señuelos rompen $289$ y $623$ & \stproved\\
Prop. \ref{prop:liftcount}, igualdad si $\Phi\ne0$ & mismo &
prueba, \S\ref{sec:lift}; control $309/329$ y $791/791$ & \stproved\\
Cor. \ref{cor:local}, la forma local & mismo &
prueba, \S\ref{sec:lift}; control $1696/1696$ y $4074/4074$ & \stproved\\
Obs. \ref{rem:skeleton}, signo y torsión en los puntos forzados & mismo &
$398/398$ y $1176/1176$; tres ansatz fallan & \stverif\\
Prob. \ref{prob:instrument}: el lugar dentro del rango de Littlewood &
$(t,r)=(2,2),(2,3),(4,2)$ & $1$ de $164$, $494$, $494$, las tres el
Teor. \ref{thm:stable}(b) & \stverif\\
\end{longtable}
\endgroup

\noindent Los dos grupos de filas de la \S\ref{sec:zeros} son el único sitio del artículo donde
el estado de una fila no se lee en su enunciado. El primero es la maquinaria y hasta dónde llega: el
Teorema \ref{conj:crit} cierra $t=2$ para todo $r$, y la única fila en \stext\ es el apoyo ajeno
del que depende; debajo, los pasos extremales, después el Teorema \ref{thm:suff}, que alcanza todo
$t$ y todo $r$ y no necesita ningún apoyo externo; y por último la Conjetura \ref{conj:general}, en
\stverif\ porque es el único enunciado de aquí que está medido y no demostrado. Sus dos subfilas
son lo que hace que la medida merezca citarse: el criterio se contrasta contra los dos teoremas que
ya deciden sus propios rangos, y a su lado corre un señuelo que lee la condición sobre $\beta$ en
vez de sobre $\mathcal{S}$, y falla en todas partes. El segundo grupo es el mismo lugar comprobado
directamente contra las definiciones, rama a rama, que es otro camino de código y no una
reformulación. Las filas por debajo de los dos son las identidades y la reducción sobre las que se
apoya la sección. En la columna, \stproved\ significa que aquí se da una demostración o se cita, y
\stverif\ que el enunciado se comprueba de forma exhaustiva sobre el rango indicado y no se
demuestra.
...
El artículo cede terreno en una docena de sitios distintos --- dentro de una demostración, después de
un lema, en una observación ---, y quien quiera saber qué está tomado prestado no debería tener que
buscarlos uno a uno. Esta sección es ese registro, en dos listas y no en una, porque las dos se leen
por razones distintas. Nada de lo que hay aquí es nuevo: cada fila apunta a un enunciado de arriba, y
donde el cuerpo ya dice de quién es un argumento, esta tabla lo repite en vez de sustituirlo.

\subsection*{Lo que usamos}
\begingroup\rowcolors{2}{hband}{white}\footnotesize\setlength{\tabcolsep}{4.0pt}
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.60\linewidth}%
                   >{\raggedright\arraybackslash}p{0.34\linewidth}@{}}
\toprule
enunciado & de quién es\\
\midrule
\endfirsthead
\multicolumn{2}{@{}l}{\footnotesize\itshape Lo que usamos, continuación}\\[1pt]
\toprule
enunciado & de quién es\\
\midrule
\endhead
la factorización por $t$-cociente en una órbita de raíces de la unidad, y la contabilidad de
$\beta$-conjuntos sobre la que se apoya toda la línea & Littlewood \cite{Lit50}; Ayyer--Kumari
\cite{AK22}
\stext\\
la factorización en un alfabeto puramente recíproco, para formas rectangulares &
Ciucu--Krattenthaler \cite{CK09} \stext\\
lo mismo para formas autocomplementarias, con el punto fijo $+1$, y la obstrucción de paridad en un
rectángulo $a\times b$ & Ayyer--Behrend \cite{AB19} \stext\\
$o_\nu(A)=sp_\nu(z)$ --- \emph{el Lema \ref{lem:AtoSp} entero} & Ayyer--Kumari \cite{AK25}: la
composición de su (2.13) con su Lema 3.2. Lo encontramos calculando y lo pasamos por el gate después
\stext\\
los caracteres universales, y la forma de la especialización de tipo $D$ & Koike--Terada \cite{KT87}
\stext\\
la regla de modificación simpléctica que usamos en la \S\ref{sec:unstable} & King \cite{King71}
\stext\\
el retorcimiento: el carácter de una clase lateral de un grupo desconectado es un carácter del
álgebra plegada & Jantzen; Kumar--Lusztig--Prasad \cite{Jantzen,KLP} \stext\\
la regla de restricción de Littlewood, y la involución asociada $[\nu^*]=[\nu]\otimes\det$ &
Littlewood \cite{Lit50}; clásicas \stext\\
la extensión de la regla de restricción que cubre todos los parámetros --- de modo que el espacio de
relaciones entre los $s_\nu$ en un alfabeto recíproco \emph{no} está sin describir &
King \cite{King71}; Koike--Terada \cite{KT87}; Enright--Willenbring \cite{EW04} \stext\\
$S_bS_{b'-1}-S_{b'}S_{b-1}=-S_{b-b'}$ & d'Ocagne \cite{Doc}, para Chebyshev de segunda especie;
clásica \stext\\
que la forma inicial de un determinante es la suma con signo sobre los óptimos de un problema de
asignación, y que la matriz es singular exactamente cuando dos óptimos empatan & álgebra lineal
tropical, estándar \cite{TropCramer} \stext\\
el levantamiento al numerador de la Definición \ref{def:lift}, la razón para preferirlo a
una involución sobre $J_\lambda$, la corrección del recuento de puntos fijos de igualdad a
desigualdad, y la observación de que una transversal lleva dos monomios & exploración asistida
por IA; véase la \S\ref{sec:disclosure} \stext\\
los valores del carácter en un elemento de orden dos & Karmakar \cite{Kar24} \stext\\
que una involución que invierte el signo reduce una suma signada a sus puntos fijos, y con ello el
Lema \ref{lem:fixbound} & estándar \stext\\
el último eslabón de $\Phi_t=0\Rightarrow$ (ii), y su forma con $n$ factores & Purbhoo--van
Willigenburg; Rajan \cite{PvW,Rajan} \stext\\
\bottomrule
\end{longtable}
\endgroup

\subsection*{Lo que es nuestro}

Una cosa hay que decirla antes de la tabla y no dentro. El alfabeto es lo que este artículo aporta
antes que cualquier enunciado suyo: las extensiones de raíces de la unidad repasadas arriba añaden más estructura de raíces de la
unidad, y la línea del par recíproco necesita formas autoduales; adjuntar un par recíproco
\emph{libre} a una órbita completa es el paso que, hasta donde sabemos, no aparece en la literatura
de factorización que aquí repasamos, y todo lo de abajo es lo que ese paso vuelve contestable.
...
\midrule
\endfirsthead
\multicolumn{2}{@{}l}{\footnotesize\itshape Lo que es nuestro, continuación}\\[1pt]
\toprule
enunciado & estado\\
\midrule
\endhead
Teorema \ref{thm:main} y su demostración, Lemas \ref{lem:L1}--\ref{lem:L5} & \stproved; el desarrollo
de Laplace por las filas congeladas es elemental, y lo nuestro es el recuento de residuos con exceso
dos, el colapso y el signo\\
Lema \ref{lem:L2} & \stproved; el determinante que allí se evalúa es de una especie que Ciucu y
Krattenthaler tratan, y la evaluación no es el contenido --- lo es el bloque congelado al que se
aplica\\
Corolario \ref{cor:zero} & \stproved; contesta la pregunta en la forma en que Prasad la planteó, que
es por lo que se enuncia aparte\\
Proposiciones \ref{rem:lattice}, \ref{prop:fibrelattice} y Corolario \ref{cor:fibregf} --- el estrato
de dos clases es un retículo, con función generatriz explícita & \stproved; nuestro\\
Proposición \ref{prop:minimal}, Proposición \ref{rem:sign} --- el multiconjunto y el signo son
exactamente lo que se ve & \stproved; nuestro\\
Proposición \ref{rem:involution}: las rachas alternan & \stproved; \emph{la parte (i) es de
Macdonald \cite[p.~91]{Mac95}, no nuestra}, y el artículo lo dice donde la proposición está\\
Teorema \ref{thm:extra}, Lema \ref{lem:single} --- independencia en el lugar recíproco & \stproved;
extiende un criterio de Ayyer--Kumari, y acota dónde se aplica su teorema\\
Teorema \ref{thm:zeros}: las dos ramas & \stproved; la rama (a) es una línea --- con $\beta$ de
paridad constante el bialternante factoriza por $A^2$ --- y \emph{la mitad autocomplementaria es un
corolario corto} de la identidad de complementación estándar sobre la familia de índices que Ayyer y
Behrend señalan. Lo que esta fila \emph{no} lleva es el recíproco; que no se anula nada más se prueba en las filas
de abajo, y por fin para todo $r$ en el Teorema \ref{conj:crit}\\
Teorema \ref{thm:r1}, Teorema \ref{thm:stable} --- el recíproco en $r=1$, y dentro del rango de
Littlewood para todo $r$ & \stproved; nuestro, y no hemos encontrado esta mitad enunciada en las fuentes repasadas\\
Corolario \ref{cor:dettwist}: rigidez bajo el giro por el determinante, un bicondicional para todo
$r$; y que desde $r\ge2$ el extremo de orden dos deja de decidir la curva &
\stproved; nuestro. Que el extremo deje de decidir la curva es el hallazgo, y $\lambda=(5,4,3,1)$ es
el primer testigo\\
Lema \ref{lem:AtoSp} & \stext; \emph{no es nuestro} --- véase la primera tabla; se enuncia aquí porque el
argumento lo necesita, y su demostración es suya\\
Lema \ref{lem:Arefl}: el menor reflejado & \stproved; la reflexión que se usa es la que Ayyer y
Behrend ya manejan\\
Lema \ref{lem:L3gen}, Lema \ref{lem:step}, Teorema \ref{thm:Ggen} --- la descomposición de Laplace y
el conjunto extremal para $r$ general & \stproved; nuestro. La cota $|G|\le2$ es el enunciado
tropical de arriba especializado a nuestra matriz, y lo que reclamamos es sólo la \emph{razón} de
ella --- que un empate obliga a $t$ par y pone los dos incrementos empatados en las clases $i$ e
$i+t/2$\\
que la factorización en grado máximo es $[A(T)]_{\mathrm{top}}=a_H(z)a_L(1/z)$ & \stproved, y
\emph{elemental}: una línea de Laplace y una cuenta de grados. No reclamamos nada de ella\\
que el emparejamiento consecutivo es el único emparejamiento de peso mínimo sobre una recta & \stext;
folclore; un argumento de intercambio de una línea, y no encontramos lema citable\\
Teorema \ref{thm:suff}, Corolario \ref{cor:necgen} --- la dirección suficiente para todo $t$ y $r$, y
una condición necesaria en $t$ par & \stproved; nuestro\\
Lema \ref{lem:signformula}, Lema \ref{lem:signrefl} --- los signos de la reflexión parcial &
\stproved; nuestro\\
Conjeturas \ref{conj:general}, \ref{conj:gcom}, \ref{conj:rank2} & \stverif; abiertas, y medidas; los rangos
están en \S\ref{sec:verif}\\
Proposición \ref{conj:rank}: el rango es el invariante equivocado & \stproved; nuestra, y es un
negativo --- quita un invariante candidato en vez de aportar uno\\
Proposición \ref{prop:wrongset}, Lema \ref{lem:liftid}, Proposición \ref{prop:liftcount} y
Corolario \ref{cor:local} --- dónde no puede vivir una involución así, y dónde sí
& \stproved; \emph{la construcción sobre la que se enuncian no es nuestra}
---véase la primera tabla--- y nuestro es haberlo medido\\
Observación \ref{rem:skeleton} --- lo que fuerzan los espacios de peso de tamaño uno &
\stverif; nuestra\\
la medida de dentro del Problema \ref{prob:instrument}: el lugar de anulación corta
el rango de Littlewood en una sola forma, y esa forma ya es un teorema & \stverif;
nuestra, y es un negativo --- dice de qué especie tiene que ser el instrumento que la
pregunta necesita, no cuál la contesta\\
los enunciados restantes --- Corolarios \ref{cor:chars}, \ref{cor:enum}, \ref{cor:sizes},
\ref{cor:uniquegen}, \ref{cor:midgen}, \ref{cor:oddgen}, \ref{cor:reflectgen}, \ref{cor:Ctaugen},
\ref{cor:whenminus}, \ref{cor:suffgeom}; Proposiciones \ref{prop:quotient}, \ref{prop:rect},
\ref{conj:iso}, \ref{prop:orbitgen}, \ref{prop:translgen}, \ref{prop:centregen}, \ref{prop:A1gen};
Lemas \ref{lem:L3}, \ref{lem:L4}, \ref{lem:nonstd}, \ref{lem:reflgen}, \ref{lem:dictgen},
\ref{lem:reflact}; y Teorema \ref{conj:crit} & \stproved; nuestros. Son la maquinaria interna de las
demostraciones de arriba, y se listan por nombre para que esta tabla cubra \emph{todos} los
enunciados del artículo y no sólo los memorables. Los hechos clásicos que invocan --- la regla de
restricción de Littlewood y su rango, las reglas de modificación --- son los cedidos en la primera
tabla\\
\bottomrule
\end{longtable}
\endgroup
...
\noindent
Tres negativos merecen enunciarse como negativos, porque cada uno costó una búsqueda. La
$(z_1,z_2,k)$-asimetría de Kumari \emph{no} clasifica nuestro lugar de anulación. No hay análogo de
d'Ocagne en rango $r$ --- sería una forma cerrada para $s_\mu$ en un alfabeto recíproco para todo
$\mu$, y la literatura sólo la da para formas especiales. Y un criterio de anulación sobre
$O(N)\setminus SO(N)$ --- que es de lo que trata la \S\ref{sec:zeros} --- no lo encontramos bajo
ninguna de dos búsquedas con vocabularios distintos.
...
relleno lo demostrado aquí, gris el único apoyo ajeno, discontinuo lo abierto. La rama del \emph{medio} es
el argumento extremal de las \S\S\ref{sec:extremal} y \ref{sec:rigidity}, que cierra el criterio
en $t=2$ para todo $r$. La de \emph{abajo} es la reducción de Littlewood, cuyo certificado refuta
la Proposición \ref{conj:iso} ---un contraejemplo, o sea una demostración---, de modo que ya no
toca el criterio sino la cuestión aparte de explicar los $C_\mu$, que es el Problema
\ref{prob:instrument}. La de \emph{arriba} arranca de una hipótesis y no del desarrollo, y es la
única que alcanza todo $t$ y todo $r$: allí la reflexión empareja todos los términos a la vez y no
sólo los dos maximizadores. Las tres se encuentran en la única implicación que falta, que es lo
que hace de la Conjetura \ref{conj:general} una conjetura. Las cajas nombran sus enunciados, y los
cuatro números que sí llevan se leen del \texttt{.aux} al dibujar, de modo que no pueden
desfasarse cuando la numeración lo haga; \texttt{check\_figrefs.py} comprueba que cada uno sigue
resolviendo, y a un enunciado del tipo correcto.
...
de un programa que no hemos construido nosotros.

\emph{El análogo simpléctico clásico no está abierto, y deberíamos haberlo dicho aquí y no sólo
en el Teorema 2.2.} La Observación \ref{rem:notacase} de más arriba compara nuestro alfabeto con el
\cite[Teorema 2.2]{Kum24}, que es el enunciado de $GL$; pero el \cite[Teorema 2.8]{Kum24} del
mismo artículo evalúa $sp_\lambda(X,\omega X,\dots,\omega^{t-1}X,\,y)$, y en un carácter
simpléctico adjuntar $y$ es adjuntar el par recíproco $\{y,y^{-1}\}$. Ésa es la mitad simpléctica
de este problema, contestada, con criterio de anulación --- su $(z_1,z_2,k)$-asimetría del
$t$-núcleo ---, y hemos contrastado ese criterio con la anulación del carácter simpléctico
universal sobre $\mu_t\cup\{z,z^{-1}\}$: coincide en $285$ de $285$ formas en $t=3,5,7$, en las
cuatro casillas \stverif. La distinción que sobrevive no es de rango sino de objeto: su
$sp_\lambda(W)$ es el carácter clásico, cuyo alfabeto de autovalores es $W$ junto con sus
inversos, así que en $X=(1)$ la órbita está \emph{doblada}, y el carácter universal sobre la
órbita tomada una vez es otra función --- los valores difieren en $155$, $96$ y $51$ de $219$
formas, mientras que el lugar de anulación coincide en $219$ de $219$ \stverif. Así que lo que
este problema sigue pidiendo son los casos ortogonales, y la brecha doblado/sin doblar en el
simpléctico. \emph{El resto de este problema es una medición, y se da como tal y nada más.} Desarrollando los caracteres universales de Koike--Terada en la base de Schur y evaluando
...
como para que adjuntarla sea un par de mapas de letra y no un operador.
\emph{El artículo compañero \cite{PaperII} contesta esto en sentido débil, y lo dice.} Exhibe una
aplicación que lleva el objeto con todos los pares libres al objeto con la órbita congelada ---una
construcción para $t$ par y otra distinta para $t$ impar, quedando abierta allí una única que dé
las dos---. Sus dos salvedades son las que dejan en pie la pregunta de arriba: la aplicación no es
una Verschiebung y no se afirma que factorice ninguna, y está montada a posteriori a partir de una
ramificación seguida de una evaluación, y no de una operación sobre funciones simétricas. Lo que
aquí se pide es lo segundo.
\end{problem}

\begin{remark}[qué del Problema \ref{prob:versch} está ya saldado, y una corrección]\label{rem:composes}
Una versión anterior de este problema afirmaba que no conocíamos nada que actuara sobre las dos
mitades a la vez. Era falso, y el contraejemplo está en la referencia que el propio problema
nombra para la mitad torcida: Albion \cite[\S6.1]{Alb23} define la Verschiebung con una letra
libre adjuntada, $\varphi^q_t h_{at+b}:=q^b\sum_k q^{kt}h_{a-k}$, y su Proposición 6.2 la evalúa
sobre $s_\lambda$. El caso de una letra estaba publicado. Lo que faltaba es el del \emph{par}, y
ahí los dos formalismos sí componen:
\begin{equation}\label{eq:composes}
s_\lambda\bigl(\mu_t\cup\{z,z^{-1}\}\bigr)=\sum_{\mu}\operatorname{sgn}_t(\mu)\,
s_{\lambda/\mu}(z,z^{-1}),
\end{equation}
con la suma sobre los $\mu\subseteq\lambda$ de $t$-núcleo vacío y con toda componente del
$t$-cociente de a lo sumo una fila. No es un teorema nuevo: es el Teorema 3.1 de Albion evaluado
en $1$, donde $s_\nu(1)=1$ si $\ell(\nu)\le1$ y $0$ en otro caso. Se deja escrito porque contesta
la última frase del problema --- las dos mitades componen, y la composición es explícita --- y
porque una pregunta no debe quedarse en pie sobre una premisa que su propia referencia refuta.
Contrastado con el valor directo, $714/714$ en $t=2$ y $2001/2001$ en $t=3$; el señuelo sin la
condición de núcleo vacío falla $709$ y $2001$ veces, el señuelo con $(z,z)$ en lugar de
$(z,z^{-1})$ falla $664$ y $1496$, y el señuelo sin $\operatorname{sgn}_t(\mu)$ falla $665$ y
$1884$ \stverif. Lo que \eqref{eq:composes} \emph{no} da es un operador: es una identidad de
ramificación, no una factorización de $\varphi_t$ por un álgebra mayor, y el problema de arriba
pide lo segundo.
\end{remark}
...
escalar de un conjunto cerrado por inversión. Esa
hipótesis es más débil de lo que parece, y (D3) es lo que mide cuánto. Una clase lateral $c\zt$ es ella misma
cerrada por inversión exactamente cuando $c^{2}\in\zt$, porque $(c\zt)^{-1}=c^{-1}\zt$; y (D3) lo cumple
para todo $t$, ya que su $\zeta'=e^{\pi i/t}$ tiene $(\zeta')^{2}=\zeta$. Su alfabeto son las potencias
impares de una raíz $2t$-ésima, y la inversión conserva la paridad del exponente. Y sin embargo (D3)
rompe la identidad, en $217$ de las $600$ formas barridas arriba. Así que la cerradura por inversión
no sobrevive como hipótesis entera ni siquiera proyectivamente: lo que a (D3) le falta es la otra
mitad de la \S\ref{sec:sharp}, que las letras congeladas sean una órbita \emph{entera} y no un coset
de una. Una forma proyectiva de la Conjetura \ref{conj:rank2} tiene que llevar las dos, y es (D3)
quien enseña que son independientes.
...
rango estable y sin regla de modificación. Jang--Kwon \cite{JK20} calculan esa misma multiplicidad,
y su forma es la más afilada para lo que aquí hace falta: es una cuenta manifiestamente no negativa
de tablas de Littlewood--Richardson con bandera. El obstáculo aquí es precisamente que nuestros
$C_\mu$ llevan signos, y lo que la \S\ref{sec:unstable} deja sin cerrar es una igualdad
$m_\nu(\lambda)=m_{\nu^{*}}(\lambda)$ entre dos multiplicidades exactamente de esa clase, en el
rango $\ell(\lambda)>N/2$ donde la regla de Littlewood deja de aplicarse. Una expresión sin restas
para cada lado es de donde querría partir una prueba allí. Nuestros $C_\mu$ de \eqref{eq:Cmu} son
sumas con signo de
...
verla contestada.
\emph{Desde que este problema se escribió, el artículo compañero \cite{PaperII} ha dado un
instrumento de la especie que pide.} Allí los coeficientes se calculan como multiplicidades de
ramificación simpléctica contra un filtro en forma cerrada, y los dos factores se calculan sin
desarrollar un solo monomio; en el lado impar la matriz de ramificación sale del numerador por
completo, y esa mitad queda como un recuento signado de transversales. Dos límites son suyos, y son
la razón de que este problema siga abierto: no explica la cancelación y no dice que lo haga, y lo
que entrega en lugar de una explicación es una única conjetura afilada. Damos el puntero y no el
contenido.
Una medida decide cuál es el instrumento al que hay que ir, y es un negativo. Por \eqref{eq:Cmu} la
anulación en $r\ge2$ es la igualdad $m_\nu(\lambda)=m_{\nu^{*}}(\lambda)$ para toda $\nu$, así que
cabe preguntar cuánto de ese lugar cae dentro del rango donde la regla de Littlewood calcula esas
multiplicidades. Casi nada: sobre $(t,r)=(2,2)$, $(2,3)$ y $(4,2)$ ---$164$, $494$ y $494$ formas con
$\ell(\lambda)\le N/2$--- exactamente una forma en cada caso cumple la igualdad, y en las tres es el
rectángulo de anchura impar con $N/2$ filas \stverif. Esa forma no es un residuo que deje el
barrido: es la rama (b) del Teorema \ref{thm:stable}, que la demuestra. De modo que dentro del rango
de Littlewood la pregunta no tiene más contenido que un teorema que ya está, y los coeficientes por
cuya cancelación pregunta \eqref{eq:Cmu} viven donde la regla no se aplica. Por eso un instrumento
sin restricción de rango estable no es aquí una comodidad sino un requisito, y es lo que convierte
las dos reglas de arriba en aquellas a las que hay que ir.
\end{problem}
...
\emph{Un negativo del artículo compañero \cite{PaperII} corresponde aquí, porque es fácil
confundirlo con una respuesta a esta pregunta.} Lo que allí se mide y falla es el tamiz cíclico en
el parámetro \emph{recíproco libre}. La pregunta de arriba es la otra ---una acción cíclica en la
dirección congelada, refinada por el peso libre--- y nada de ese artículo la toca. Registramos la
distinción para que no se confundan, que es la razón por la que ese artículo también la registra.
\end{problem}

\begin{problem}[el toggle sobre el levantamiento]\label{prob:toggle}
La involución de la \S\ref{sec:enum} queda ensamblada en $t=2$ y en ningún otro sitio, y la
\S\ref{sec:lift} dice por qué: sobre $J_\lambda$ el requisito es aritméticamente imposible para un
tercio de las formas del rango, y el conjunto donde no lo es es el levantamiento
$\widehat J_\lambda$ de la Definición \ref{def:lift}. Lo que falta es la involución misma ---que
Enunciada de modo que los objetos casen: una inyección
$\eta_\lambda:\mathcal L(\lambda)\hookrightarrow\widehat J_\lambda$ que conserve peso y signo,
junto con una involución $\iota$ sobre $\widehat J_\lambda$ que conserve el peso, invierta el signo y
cuyo conjunto de puntos fijos sea exactamente $\operatorname{im}\eta_\lambda$. Para $\Phi_t\ne0$ el Corolario \ref{cor:local} fija el objetivo de forma independiente en cada
espacio de peso, así que puede construirse espacio a espacio; el caso nulo hay que tratarlo aparte; por la Observación \ref{rem:skeleton} sus valores ya están forzados en $398$ y
$1176$ de esos espacios, y cualquier solución tiene que coincidir con ellos ahí. La forma que hay
que intentar es un toggle sobre un modelo de caminos en el que las elecciones de $\Omega_t$ sean
pasos, que es el marco de \cite{Fulmek}: allí Cauchy--Binet y Laplace, los dos lados que hay que
emparejar, viven en un mismo dibujo. Lo que no tenemos es una regla candidata. Lo que sí tenemos es
que nada numérico se interpone, y eso no era cierto antes del levantamiento.
\end{problem}

La distinción que mantenemos en todo el texto es entre una \emph{conjetura}, que es un enunciado que
...
%======================================================================
\section{Cierre: un alfabeto, leído de tres maneras}\label{sec:closing}

La introducción llamó a \eqref{eq:object} un elemento del toro de $O(N)$ con una única dirección
libre, y dijo que los tres resultados de aquí son tres lecturas de esa frase. Es fácil leerlos como
tres resultados sueltos, así que cerramos diciendo qué los separa y qué queda después de ellos.

\emph{El valor.} El Teorema \ref{thm:main} es lo que vale la dirección libre: tres factores sobre un
denominador fijo, para todo $t$ y todo $\lambda\in\PP_N$, sin hipótesis sobre la forma. Las dos
mitades del alfabeto lo producen juntas y ninguna lo produce sola. El perfil de residuos pone el
esqueleto del lado de la órbita ---resuelve la rama degenerada y nombra las dos clases de exceso---
y el par recíproco es quien acopla esas clases en el triple de intervalos, mientras el orden de la
palabra beta da el signo. Quítese el par y el enunciado degenera en el de Littlewood; consérvese, y
el perfil sigue sin alcanzar el triple, porque ya en $t=2$ el único perfil de dos clases lleva diez
valores distintos de $d_3$ sobre $|\lambda|\le16$.

\emph{El invariante.} El mismo teorema leído del revés dice que la evaluación no puede ver de
$\lambda$ más que un triple de intervalos y un signo: dos particiones de tamaños cualesquiera que
coincidan ahí toman el mismo valor. En el estrato de dos clases, el Corolario \ref{cor:fibregf}
escribe la función generatriz de lo que el triple olvida; el estrato de tamaño tres y el degenerado
son el Problema \ref{prob:fibres}, y por la Observación \ref{rem:signsees} el signo ve direcciones
que el triple no ve. Ésa es la parte que esperamos que sobreviva a la fórmula, porque es un
enunciado sobre el alfabeto y no sobre la demostración.

\emph{El lugar de anulación.} Pasado un par, la fórmula de tres factores del Teorema \ref{thm:main} se ha
ido ---(D1) ya da un contraejemplo en $r=2$, y que no exista tal fórmula para ningún $r\ge2$ es la
Conjetura \ref{conj:rank2} y no un teorema--- y el conjunto de ceros no. Lo que la mitad de inversión aporta ahí es un segundo
mecanismo, y en $t=2$ también una lectura: por el Teorema \ref{conj:crit} el carácter se anula exactamente cuando el
conjunto beta tiene paridad constante, que es un enunciado sobre residuos, o la forma es
autocomplementaria de anchura impar, que es complementación; y el Corolario \ref{cor:dettwist} lee ese criterio sobre el grupo
como rigidez frente al giro por $\det$. El determinante $(-1)^{t+1}$ decide en qué componente de
$O(N)$ vive el alfabeto, y con ella qué lectura hay disponible; no decide por sí solo qué $\lambda$
se anulan. Que los dos mecanismos son de verdad dos lo enseña el Corolario \ref{cor:oddgen}: todo
$t$ impar y todo $r$ sale sólo del argumento extremal, sin apoyo externo y sin apelar a la
complementación.

\emph{Lo que queda.} Las dos razones que nombró la introducción no acaban en el mismo sitio. La
mitad de órbita está agotada sobre este alfabeto: el Teorema \ref{thm:main} la evalúa en forma
cerrada para todo $t$, y
el Teorema \ref{thm:extra} enseña que dentro del problema de independencia de dos filas hay
exactamente una familia más que se comporta así. La de inversión no. Ha bastado para decir bastante
sobre \emph{dónde} se anula el carácter ---el Teorema \ref{conj:crit} en $t=2$ para cualquier número
de pares, el Teorema \ref{thm:suff} una implicación para todo $t$ y todo $r$--- pero no nos ha dicho
qué \emph{es} el carácter ahí con más de un par, y la otra implicación es la Conjetura
\ref{conj:general}. El valor no deja nunca de ser un valor de carácter: es $s_\lambda$ evaluado en
un elemento del toro en todo momento. Lo que deja de valer pasado un par es que el desarrollo
simpléctico plegado sea siempre un carácter genuino; desde dos pares aparecen ejemplos propiamente
virtuales, y eso es un enunciado sobre el desarrollo y no sobre el objeto. Lo que el artículo entrega
es más pequeño que cualquiera de las dos mitades y más portable: una forma cerrada que lleva su
signo, lo que convierte a esta familia en un banco de pruebas para cualquier implementación de
caracteres torcidos o universales; un test de anulación que cuesta dos restas sobre el conjunto beta;
y una enumeración en la que el parámetro sigue libre. Quien busque el alfabeto siguiente debería
buscar el que haga la segunda razón tan completa como la primera ha llegado a ser aquí; y a la vista
de la \S\ref{sec:sharp} no se encontrará añadiendo letras a éste.
...
aplique al alfabeto entero, así que funciona igual sobre una parte de él. El Teorema
\ref{thm:suff}, y con él la parte de la \S\ref{sec:zeros} que va por reflexión, se apoya en
esa maniobra; el Corolario \ref{cor:oddgen} no, y se prueba sólo con el argumento extremal.

\subsection*{Código, datos y declaración}\label{sec:disclosure}\label{sec:disclosure}
...
están también en \url{https://github.com/karlesmarin/schur-orbit-and-reciprocal-pair}. Se ha usado IA
generativa como asistente de investigación durante todo el trabajo. Las matemáticas, y la
responsabilidad sobre ellas, son del autor.
...
\bibitem[Ful12]{Fulmek} M.~Fulmek, \emph{Viewing determinants as nonintersecting lattice
paths yields classical determinantal identities bijectively}, Electron. J. Combin. \textbf{19}
(2012), no.~3, Paper 21; arXiv:1010.3860.
...
\bibitem[GM81]{GM81} A.~M.~Garsia and S.~C.~Milne, \emph{A Rogers--Ramanujan bijection},
J. Combin. Theory Ser. A \textbf{31} (1981), 289--339.
...
\bibitem[JK21]{JK20} I.-S.~Jang y J.-H.~Kwon, \emph{Flagged Littlewood--Richardson tableaux
and branching rule for classical groups}, J. Combin. Theory Ser.~A \textbf{181} (2021),
Paper No.~105419; arXiv:1908.11041.
...
\bibitem[Mar26b]{PaperII} C.~Mar\'in, \emph{Schur polynomials twisted by roots of unity
and reciprocal pairs: torsion filters, fusion quotients, and total unimodularity at odd
order}, arXiv:2608.18302.
