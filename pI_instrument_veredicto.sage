...
for (T, R) in CASOS:
    ...
    print("")
    sys.stdout.flush()
    # int() y no list() a secas: las partes son Integer de Sage y json las rechaza.  La corrida del
    # 19 de agosto murio aqui con un TypeError DESPUES de imprimirlo todo, asi que el .txt archivado
    # terminaba en un traceback y el DUMP.json quedo cortado en 60 bytes.  Los numeros eran
    # correctos y el archivo no lo parecia.
    resumen["t=%d,r=%d" % (T, R)] = dict(n_lams=int(len(lams)),
                                         sobreviven=[[int(p) for p in x] for x in sobreviven],
