...
- *Orders generated by character values of $\mathrm{SU}(n)$, $\mathrm{Sp}(2m)$ and $\mathrm{GL}(n)$ at elements of finite order: the index depends only on the segment of exponents* — the segment invariance, the support criterion and the
  quadratic bridge $B=R\oplus\theta R$ that `puente` implements.
  Concept DOI [10.5281/zenodo.22847476](https://doi.org/10.5281/zenodo.22847476).
- *Zeros of weighted partial sums of completely multiplicative functions: Dirichlet characters, the Euler factor at 2 and Conrey's sine series* --- the vanishing components on
  the diagonal and the rows, which `ceros` and `fila` implement.
  Concept DOI [10.5281/zenodo.22847374](https://doi.org/10.5281/zenodo.22847374).
