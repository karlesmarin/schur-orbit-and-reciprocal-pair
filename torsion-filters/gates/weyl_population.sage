...
CASOS = [tuple(sorted(c, reverse=True)) + (0,)
         for c in _it.combinations(range(1, 12), N - 1)][:70]

AGG = {"n": 0, "ctrl": 0, "bal": 0, "rep": {}, "nv": {}, "desbal": []}
print("=" * 118)
...
for b in CASOS:
    ...
    npos = sum(1 for _, _, tv, _ in vivos if tv > 0)
    nneg = sum(1 for _, _, tv, _ in vivos if tv < 0)
    ...
    AGG["n"] += 1
    AGG["ctrl"] += 1 if suma == A[mm] else 0
    AGG["bal"] += 1 if npos == nneg else 0
    AGG["rep"][(npos, nneg)] = AGG["rep"].get((npos, nneg), 0) + 1
    AGG["nv"][len(vivos)] = AGG["nv"].get(len(vivos), 0) + 1
    if npos != nneg and len(AGG["desbal"]) < 8:
        AGG["desbal"].append((tuple(int(v) for v in b), npos, nneg, int(A[mm])))
...
print("")
print("  FORMAS con mu_max unico: %d | control suma == A: %d | signos BALANCEADOS: %d"
      % (AGG["n"], AGG["ctrl"], AGG["bal"]))
print("  reparto (n+, n-): %s" % dict(sorted(AGG["rep"].items())))
print("  numero de supervivientes: %s" % dict(sorted(AGG["nv"].items())))
if AGG["desbal"]:
    print("  DESBALANCEADAS (beta, n+, n-, A): %s" % str(AGG["desbal"]))
print("")
print("=" * 118)
...
print("     los atomos.")
json.dump(RES, open("weyl_population_DUMP.json", "w"), indent=1)
print("=" * 118)
print("DONE")
