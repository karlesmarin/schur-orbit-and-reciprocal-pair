...
def dividir(nu, t, rr, tope=20000):
    """resuelve  c . Delta_t = nu  por sustitucion hacia atras desde el peso mas alto.
       Devuelve (c, resto, max|c| visto).  Esto es la DIVISION, hecha solo con datos GKRS."""
    ...
    for _ in range(tope):
        ...
        y = cabeza(P)
        cand = None
        for eps in itertools.product((1, -1), repeat=rr):
            e = enderezar(tuple(int(y[j]) - t * eps[j] for j in range(rr)))
            if e is None:
                continue
            D = desplazar(e[0], t)
            if D and cabeza(D) == y:
...
for (t, r, cota) in CASOS:
    ...
    agg = {"n": 0, "nt": 0, "T1": 0, "nu_rep": Counter(), "mu_w_distintos": 0, "sin_colision": 0,
           "T4": 0, "c_rep": Counter(), "D1": 0, "D2": 0, "fallos": [], "colisiones": [],
           "signo": Counter(), "T5": 0, "T5resto": 0, "peor_div": 0, "div_rep": Counter()}
    ...
    for Lam in dominantes(Rp, cota):
        ...
        # el signo GLOBAL es una convencion del orden de la diferencia de semiespines: se detecta,
        # no se supone.  Se exige que sea el MISMO para toda la familia.
        ...
        neg = {k: -v2 for k, v2 in lhs.items()}
        ok1 = (lhs == nu) or (neg == nu)
        if lhs and lhs == nu:
            agg["signo"][+1] += 1
        elif lhs and neg == nu:
            agg["signo"][-1] += 1
        ...
        for mu, cv in c.items():
            ...
            for y, sg in desplazar(x, t + 2).items():
                d1[y] += cv * sg
        d1 = {k: v2 for k, v2 in d1.items() if v2 != 0}
        okD1 = (d1 == nu) or ({k: -v2 for k, v2 in d1.items()} == nu)
        d2 = defaultdict(lambda: 0)
        for mu, cv in c.items():
            x = tuple(2 * int(mu[j]) + rhoD2[j] for j in range(r))
            for y, sg in desplazar(x, t, con_signo=False).items():
                d2[y] += cv * sg
        d2 = {k: v2 for k, v2 in d2.items() if v2 != 0}
        okD2 = (d2 == nu) or ({k: -v2 for k, v2 in d2.items()} == nu)
        ...
        # ---- T5: LA DIVISION, hecha solo con datos GKRS (sin branching): reconstruir c desde nu
        crec, resto, peor = dividir(nu, t, r)
        cx = {}
        for mu, cv in c.items():
            cx[tuple(2 * int(mu[j]) + rhoD2[j] for j in range(r))] = cv
        ok5 = (not resto) and (crec == cx or {k: -v2 for k, v2 in crec.items()} == cx)

        agg["n"] += 1
        agg["nt"] += 1 if c else 0
        agg["T1"] += 1 if ok1 else 0
        agg["T5"] += 1 if ok5 else 0
        agg["T5resto"] += 1 if resto else 0
        agg["peor_div"] = max(agg["peor_div"], peor)
        for v2 in crec.values():
            agg["div_rep"][int(v2)] += 1
        if c:                                   # los senuelos solo cuentan donde hay algo que fallar
    ...
    print("     T1  CONTROL FATAL   c . Delta_t == nu            : %d de %d   (signo global: %s)"
          % (agg["T1"], agg["n"], dict(sorted(agg["signo"].items()))))
    print("     D1  senuelo  paso t+2                            : %d de %d  (debe ser ~0)" % (agg["D1"], agg["nt"]))
    print("     D2  senuelo  sin el signo prod eps               : %d de %d  (debe ser ~0)" % (agg["D2"], agg["nt"]))
    ...
    print("     T5  DIVISION solo con datos GKRS -> reproduce c  : %d de %d  (resto no nulo: %d)"
          % (agg["T5"], agg["n"], agg["T5resto"]))
    print("         reparto del cociente, y peor |c| de la division: %s   peor=%d"
          % (dict(sorted(agg["div_rep"].items())), agg["peor_div"]))
    if agg["fallos"]:
    ...
    RES.append({"t": int(t), "r": int(r), "mp": int(mp), "Rp": int(Rp), "cota": int(cota),
                ...
                "nt": int(agg["nt"]), "T5": int(agg["T5"]), "T5resto": int(agg["T5resto"]),
                "peor_div": int(agg["peor_div"]),
                "signo": {str(k): int(v2) for k, v2 in agg["signo"].items()},
                "div_rep": {str(k): int(v2) for k, v2 in agg["div_rep"].items()},
...
json.dump(RES, open("gkrs_L1_DUMP.json", "w"), indent=1)
print("=" * 118)
print("DONE")
