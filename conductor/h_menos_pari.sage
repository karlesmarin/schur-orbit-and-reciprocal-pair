...
# What: independent h^- = h(Q(zeta_n)) / h(Q(zeta_n)^+) via PARI bnfinit (class_number), for phi(n)<=20, n != 2 mod 4, compared to the analytic formula.
# Run (in this directory): sage h_menos_pari.sage
import sys, time
def hminus(n):
...
allok = True
for n in range(3, 90):
    if n % 4 == 2 or euler_phi(n) > 20: continue
    t0 = time.time()
    C = CyclotomicField(n)
    h = C.class_number(proof=True)
    Kp = C.maximal_totally_real_subfield()[0]
    hp = Kp.class_number(proof=True)
    ha = hminus(n)
