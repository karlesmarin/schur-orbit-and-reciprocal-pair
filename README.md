...
The last of them is the newest, deposited 19 September 2026: `segmento/` computes the ring of
character values at the **principal** element — the column `mu = 0` of the modular `S`-matrix, the
quantum dimensions — and `s-matrix-column/` computes it at **every** column, where the ring turns
out to be, locally at the height `p = n + k`, a monomial ring whose valuations form a numerical
semigroup. The index is `p` to the number of gaps, and the order is Gorenstein exactly when that
