...
Seven more follow the same line, from character values to the **ring they generate** and on to a
count that ring leaves open, each with the note in both languages and the scripts behind every number it calls *measured*:
...
| [`s-matrix-column/`](s-matrix-column) | *The order generated by a column of the SU(n)_k S-matrix is locally a numerical semigroup ring* (Part I), with its tool `P3a.py` | [10.5281/zenodo.22848529](https://doi.org/10.5281/zenodo.22848529) |
| [`s-matrix-column-ii/`](s-matrix-column-ii) | the same, **Part II**: *the gluing term, the distribution of the defect, and the classical objects at its two ends* | [10.5281/zenodo.22864266](https://doi.org/10.5281/zenodo.22864266) |
| [`two-moments-i/`](two-moments-i) | *Subsets of F_p with two prescribed moments, Part I: the pair {1,3}*, with its program `P3c.py` | [10.5281/zenodo.22886540](https://doi.org/10.5281/zenodo.22886540) |
| [`two-moments-ii/`](two-moments-ii) | the same, **Part II**: *every exponent* | [10.5281/zenodo.22886542](https://doi.org/10.5281/zenodo.22886542) |

`segmento/` computes the ring of character values at the
**principal** element — the column `mu = 0` of the modular `S`-matrix, the quantum dimensions —
and `s-matrix-column/` computes it at **every** column, where the ring turns out to be, locally at
the height `p = n + k`, a monomial ring whose valuations form a numerical semigroup. The index is
`p` to the number of gaps, and the order is Gorenstein exactly when that semigroup is symmetric.

`s-matrix-column-ii/` asks the next question: not what each column is, but how they **fit
together**. The `p`-adic valuation of the discriminant of the whole fusion ring splits into
ramification, twice the sum of the column defects, and twice a gluing term, and the only piece
without a closed form is the census `Delta(n,p) = sum of delta(T) over orbits`. Almost every
column has defect one — the proportion with `delta >= 2` is `2/p - 1/p^2` up to an explicit
error — and the two ends of that distribution turn out to be indexed by objects nobody invented
for the purpose: Bring's totally split trinomials at the top, Gauss's half-systems in the middle.
Thirty-one scripts travel with it, six of which audit the manuscript itself rather than the
mathematics.

`two-moments-i/` and `two-moments-ii/` take up a count that `s-matrix-column-ii/` wrote down and
left without a closed form: the number `C_{1,3}(n,p)` of `n`-subsets of `F_p` whose sum and sum of
cubes both vanish, which there has a main term and an error bound. Part I proves it is elementary
— a polynomial in `p` with coefficients in quadratic characters of fixed integers — exactly for
`n = 3, 4, 6`, with four closed forms. Part II replaces the cube by every exponent `k >= 3`: the
count is Artin–Tate for `n = 3` alone once `k` is not `3` or `5`, the column `n = 3` is governed
by polynomials `Q_k` whose degree plus one is the dimension of the space of modular forms of
weight `2k` for `SL_2(Z)`, and where it is not elementary the count is not PORC.
